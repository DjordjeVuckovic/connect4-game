"""
Modul u kome je implemetiran Single player vs L1 i Single player vs L2 mod
"""
...
def drawBoard(board):
    for col in range(const.COLUMNS):
        for row in range(const.ROWS):
            pg.draw.rect(screen, const.GOLD, (
            col * const.SQUARESIZE, row * const.SQUARESIZE + const.SQUARESIZE, const.SQUARESIZE, const.SQUARESIZE))
            pg.draw.circle(screen, const.DARK_GRAY, (int(col * const.SQUARESIZE + const.SQUARESIZE / 2),
                                                     int(row * const.SQUARESIZE + const.SQUARESIZE + const.SQUARESIZE / 2)),
                           const.RADIUS)
    for col in range(const.COLUMNS):
        for row in range(const.ROWS):
            if board[row][col] == 1:
                pg.draw.circle(screen, const.RED, (int(col * const.SQUARESIZE + const.SQUARESIZE / 2),
                                                   const.HEIGHT - int(row * const.SQUARESIZE + const.SQUARESIZE / 2)),
                               const.RADIUS)
            elif board[row][col] == -1:
                pg.draw.circle(screen, const.SKYBLUE, (int(col * const.SQUARESIZE + const.SQUARESIZE / 2),
                                                       const.HEIGHT - int(
                                                           row * const.SQUARESIZE + const.SQUARESIZE / 2)),
                               const.RADIUS)
    pg.display.update()
